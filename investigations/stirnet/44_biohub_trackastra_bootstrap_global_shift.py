...
r"""
Investigation 44 — BioHub Trackastra bootstrap global-motion compensation.

Scientific question
-------------------
Can a first Trackastra pass estimate BioHub global translation well enough
that a second Trackastra pass on the stabilized movie reduces track breaks?

The production-style shift is estimated only from the first-pass Trackastra
graph. Annotation-owned corrected tracks are used only as a reference for
measuring shift-estimation error and for the precise manual-Continue score.
...
B. BOOTSTRAP GLOBAL SHIFT / PASS 2
   Robust frame-to-frame global motion is estimated from unambiguous adjacent
   edges of pass 1. Raw + labels are stabilized by that estimated motion, then
   the exact same Trackastra model/mode/solver is run again.
...
* First-pass division edges are excluded from bootstrap shift estimation.
* Explicit Continue / Break / Birth annotations are the primary precise score.
* A second rough score counts unresolved interior starts/ends across all 100
  frames using the annotator's boundary rules: first/last movie frame,
  4-um boundary heuristic, plus exact six-face raster contact.
* Trackastra graph nodes are mapped directly through their `time` and `label`
  attributes; no nearest-centroid rematching is used.
* Investigation 44 defaults to CPU in the current environment because
...
From the repository root:
    ...
    python .\investigations\stirnet\44_biohub_trackastra_bootstrap_global_shift.py

Audit the annotations and bootstrap shifts without running Trackastra:

    python .\investigations\stirnet\44_biohub_trackastra_bootstrap_global_shift.py --audit-only

Re-run and replace previous Investigation-43 outputs:

    python .\investigations\stirnet\44_biohub_trackastra_bootstrap_global_shift.py --force

Keep the large temporary shifted raw/label movies for inspection:

    python .\investigations\stirnet\44_biohub_trackastra_bootstrap_global_shift.py --keep-shifted-inputs
"""
...
SCRIPT_NAME = "44_biohub_trackastra_bootstrap_global_shift"
...
class Paths:
    ...
    def shifted_raw(self) -> Path:
        return self.temp_dir / "bootstrap_shifted_raw.npy"

    @property
    def shifted_labels(self) -> Path:
        return self.temp_dir / "bootstrap_shifted_instances.npy"
...
def make_paths(args: argparse.Namespace) -> Paths:
    ...
    if not record.paths.spatial_complete(frame_count=frame_count):
        raise RuntimeError(
            "Canonical spatial inference is incomplete. Investigation 44 needs "
    ...
    if missing:
        raise FileNotFoundError(
            "Investigation 44 is missing required curated artifacts:\n  "
    ...
    return Paths(
        ...
        shifts_csv=output / "bootstrap_shifts.csv",
        shift_summary_json=output / "bootstrap_shift_summary.json",
        comparison_json=output / "comparison.json",
        break_comparison_csv=output / "break_comparison.csv",
        baseline_dir=output / "baseline",
        shifted_dir=output / "bootstrap_shift",
    ...
    *,
    ...
    for t in range(paths.frame_count - 1):
        ...
        try:
            ...
            raise RuntimeError(
                f"Could not estimate bootstrap global shift for t={t:03d}->{t+1:03d}: "
                f"{exc}"
            ) from exc
...
def _shift_result_from_pairwise(
    pairwise: np.ndarray,
    *,
    spatial_shape_zyx: Sequence[int],
    rows: pd.DataFrame,
    ...
    if pairwise.ndim != 2 or pairwise.shape[1] != 3:
        raise ValueError(f"Expected pairwise shifts (T-1,3), got {pairwise.shape}")

    frame_count = int(pairwise.shape[0]) + 1
    cumulative = np.zeros((frame_count, 3), dtype=np.float64)
...
def estimate_bootstrap_shifts(
    ...
    *,
    ...
    """
    Estimate global frame-to-frame translation from first-pass Trackastra.

    Only adjacent, unambiguous one-to-one Trackastra continuations are used:
      * source frame -> next frame only;
      * source out-degree == 1, so division parents are excluded;
      * target in-degree == 1.

    A robust median displacement with MAD residual rejection is calculated for
    ...
    for source_id, target_id in graph.edges:
        ...
        c0 = np.asarray(source["coords"], dtype=np.float64)
        c1 = np.asarray(target["coords"], dtype=np.float64)
        if c0.shape != (3,) or c1.shape != (3,):
            continue
        if not (np.all(np.isfinite(c0)) and np.all(np.isfinite(c1))):
            continue

        vectors_by_frame[t0].append(c1 - c0)

    pairwise = np.zeros((int(frame_count) - 1, 3), dtype=np.float64)
    records: list[dict[str, Any]] = []

    for t in range(int(frame_count) - 1):
        vectors = np.asarray(vectors_by_frame[t], dtype=np.float64)
        if vectors.size == 0:
            vectors = np.empty((0, 3), dtype=np.float64)
        ...
        try:
            ...
            raise RuntimeError(
                f"Could not estimate bootstrap global shift for "
    ...
    rows = pd.DataFrame(records)
    result = _shift_result_from_pairwise(
        pairwise,
    ...
    for t in range(int(frame_count) - 1):
        result.rows.loc[t, "cumulative_dz_at_to"] = float(
            result.cumulative_float_zyx[t + 1, 0]
        )
        result.rows.loc[t, "cumulative_dy_at_to"] = float(
            result.cumulative_float_zyx[t + 1, 1]
        )
        result.rows.loc[t, "cumulative_dx_at_to"] = float(
            result.cumulative_float_zyx[t + 1, 2]
        )
        result.rows.loc[t, "align_z_at_to"] = int(
            result.align_int_zyx[t + 1, 0]
        )
        result.rows.loc[t, "align_y_at_to"] = int(
            result.align_int_zyx[t + 1, 1]
        )
        result.rows.loc[t, "align_x_at_to"] = int(
            result.align_int_zyx[t + 1, 2]
        )

    return result


def compare_shift_estimates(
    bootstrap: ShiftResult,
    reference: ShiftResult,
    *,
    spacing_zyx: Sequence[float],
) -> tuple[pd.DataFrame, dict[str, Any]]:
    if bootstrap.pairwise_float_zyx.shape != reference.pairwise_float_zyx.shape:
        raise ValueError("Bootstrap/reference pairwise shift shape mismatch")

    spacing = np.asarray(spacing_zyx, dtype=np.float64)

    pair_error = bootstrap.pairwise_float_zyx - reference.pairwise_float_zyx
    pair_error_um = np.linalg.norm(pair_error * spacing[None, :], axis=1)

    cumulative_error = (
        bootstrap.cumulative_float_zyx
        - reference.cumulative_float_zyx
    )
    cumulative_error_um = np.linalg.norm(
        cumulative_error * spacing[None, :],
        axis=1,
    )

    rows: list[dict[str, Any]] = []
    for t in range(bootstrap.pairwise_float_zyx.shape[0]):
        rows.append(
            {
                "frame_from": int(t),
                "frame_to": int(t + 1),
                "bootstrap_dz": float(bootstrap.pairwise_float_zyx[t, 0]),
                "bootstrap_dy": float(bootstrap.pairwise_float_zyx[t, 1]),
                "bootstrap_dx": float(bootstrap.pairwise_float_zyx[t, 2]),
                "reference_dz": float(reference.pairwise_float_zyx[t, 0]),
                "reference_dy": float(reference.pairwise_float_zyx[t, 1]),
                "reference_dx": float(reference.pairwise_float_zyx[t, 2]),
                "pair_error_um": float(pair_error_um[t]),
                "cumulative_error_um_at_to": float(
                    cumulative_error_um[t + 1]
                ),
            }
        )

    summary = {
        "pair_error_um_median": float(np.median(pair_error_um)),
        "pair_error_um_p90": float(np.percentile(pair_error_um, 90)),
        "pair_error_um_max": float(np.max(pair_error_um)),
        "cumulative_error_um_median": float(np.median(cumulative_error_um)),
        "cumulative_error_um_p90": float(np.percentile(cumulative_error_um, 90)),
        "cumulative_error_um_final": float(cumulative_error_um[-1]),
        "reference": (
            "annotation-owned corrected current_tracks.csv; used only as an "
            "evaluation reference, not as bootstrap input"
        ),
    }
...
# =============================================================================
# Full-volume rough endpoint diagnostics
# =============================================================================


@dataclass(frozen=True)
class CuratedGeometry:
    valid_nodes: frozenset[Node]
    centers: dict[Node, np.ndarray]
    bbox_min: dict[Node, np.ndarray]
    bbox_max: dict[Node, np.ndarray]
    exact_boundary_nodes: frozenset[Node]


def build_curated_geometry(paths: Paths) -> CuratedGeometry:
    """
    Extract current curated detection geometry across all 100 frames.

    Manual instance frames override the canonical final_instances movie exactly
    as they do in the annotation session. Bounding boxes use regionprops'
    exclusive maximum convention, matching cells_all.csv.
    """
    ...
    bbox_min: dict[Node, np.ndarray] = {}
    bbox_max: dict[Node, np.ndarray] = {}
    exact_boundary_nodes: set[Node] = set()

    print("", flush=True)
    print("[diagnostics] extracting curated geometry for 100-frame endpoint audit", flush=True)

    for t in range(paths.frame_count):
        frame = load_curated_frame(paths, t, base_handle=base)

        for region in regionprops(frame):
            node = (int(t), int(region.label))
            valid_nodes.add(node)
            centers[node] = np.asarray(region.centroid, dtype=np.float64)
            bbox = np.asarray(region.bbox, dtype=np.float64)
            bbox_min[node] = bbox[:3]
            bbox_max[node] = bbox[3:]

        for instance_id in boundary_touching_instance_ids(frame):
            exact_boundary_nodes.add((int(t), int(instance_id)))
        ...
        if (
            ...
            print(
                f"[diagnostics] geometry {t + 1:3d}/{paths.frame_count}",
                flush=True,
            )
    ...
    return CuratedGeometry(
        valid_nodes=frozenset(valid_nodes),
        centers=centers,
        bbox_min=bbox_min,
        bbox_max=bbox_max,
        exact_boundary_nodes=frozenset(exact_boundary_nodes),
    )


def _heuristic_boundary_node(
    node: Node,
    geometry: CuratedGeometry,
    *,
    spatial_shape_zyx: Sequence[int],
    spacing_zyx: Sequence[float],
    boundary_margin_um: float,
) -> bool:
    center = geometry.centers.get(node)
    if center is None:
        return False
    ...
    lower_center = center * spacing
    upper_center = (shape - 1.0 - center) * spacing
    distance_um = float(np.minimum(lower_center, upper_center).min())

    bmin = geometry.bbox_min.get(node)
    bmax = geometry.bbox_max.get(node)
    if bmin is not None and bmax is not None:
        bbox_distance = np.minimum(
            bmin * spacing,
            (shape - bmax) * spacing,
        ).min()
        if math.isfinite(float(bbox_distance)):
            distance_um = float(bbox_distance)

    return distance_um <= float(boundary_margin_um)


def full_volume_endpoint_diagnostics(
    ...
    *,
    ...
    """
    Reproduce the annotation system's unresolved endpoint semantics over all
    current detections, including isolated detections absent from the solution
    graph.

    Legitimate start:
      frame 0 OR heuristic boundary OR exact raster-touch boundary.

    Legitimate end:
      frame T-1 OR heuristic boundary OR exact raster-touch boundary.
    """
    predicted_edges = set(result.predicted_edges)
    incoming = {right for _left, right in predicted_edges}
    outgoing = {left for left, _right in predicted_edges}

    starts = set(geometry.valid_nodes) - incoming
    ends = set(geometry.valid_nodes) - outgoing

    def boundary(node: Node) -> tuple[bool, bool, bool]:
        exact = node in geometry.exact_boundary_nodes
        heuristic = _heuristic_boundary_node(
            node,
            geometry,
            spatial_shape_zyx=spatial_shape_zyx,
            spacing_zyx=spacing_zyx,
        ...
        return bool(exact or heuristic), bool(exact), bool(heuristic)

    unresolved_starts: set[Node] = set()
    unresolved_ends: set[Node] = set()
    boundary_starts: set[Node] = set()
    boundary_ends: set[Node] = set()

    rows: list[dict[str, Any]] = []

    for node in sorted(starts):
        is_boundary, exact, heuristic = boundary(node)
        legitimate = int(node[0]) == 0 or is_boundary
        if legitimate:
            if int(node[0]) != 0 and is_boundary:
                boundary_starts.add(node)
        else:
            unresolved_starts.add(node)
        ...
        rows.append(
            {
                ...
                "frame": int(node[0]),
                "cell_id": int(node[1]),
                "unresolved": bool(not legitimate),
                "movie_boundary": bool(int(node[0]) == 0),
                "spatial_boundary": bool(is_boundary),
                "exact_raster_touch": bool(exact),
                "heuristic_boundary": bool(heuristic),
            }
        )

    for node in sorted(ends):
        is_boundary, exact, heuristic = boundary(node)
        legitimate = int(node[0]) == int(frame_count) - 1 or is_boundary
        if legitimate:
            if int(node[0]) != int(frame_count) - 1 and is_boundary:
                boundary_ends.add(node)
        else:
            unresolved_ends.add(node)
    ...
    # Connected-component count follows TrackAnnotationSession semantics:
    # components are undirected connectivity under active temporal edges, and
    # isolated valid detections are components too.
    import networkx as nx

    undirected = nx.Graph()
    undirected.add_nodes_from(geometry.valid_nodes)
    undirected.add_edges_from(predicted_edges)

    failure_nodes = unresolved_starts | unresolved_ends
    components_with_failure = 0
    for component in nx.connected_components(undirected):
        if set(component) & failure_nodes:
            components_with_failure += 1

    summary = {
        "valid_detections": int(len(geometry.valid_nodes)),
        "predicted_edges": int(len(predicted_edges)),
        "all_start_endpoints": int(len(starts)),
        "all_end_endpoints": int(len(ends)),
        "interior_new_endpoints": int(len(unresolved_starts)),
        "interior_broken_endpoints": int(len(unresolved_ends)),
        "total_interior_failure_endpoints": int(
            len(unresolved_starts) + len(unresolved_ends)
        ),
        "components_with_interior_failure": int(components_with_failure),
        "boundary_start_endpoints_removed": int(len(boundary_starts)),
        "boundary_end_endpoints_removed": int(len(boundary_ends)),
        "boundary_margin_um": float(boundary_margin_um),
        "exact_raster_touch_additive": True,
        "first_frame_starts_are_legitimate": True,
        "last_frame_ends_are_legitimate": True,
        "note": (
            "This is a rough full-volume tracking diagnostic, not ground truth. "
            "Unreviewed frames can still contain segmentation errors."
        ),
    }
...
def run_trackastra(
    ...
    *,
    ...
    """
    Run only the Trackastra stages needed by Investigation 44.
    ...
    Deliberately DO NOT call model.track():
      ...
      * it additionally applies the solution graph back onto the full mask
        movie, which Investigation 44 does not need;
    ...
    if mode not in {"greedy", "greedy_nodiv"}:
        raise ValueError(
            "Investigation 44's no-mask tracking path supports only "
        ...
        rows.append(
            {
                ...
                "bootstrap_shift_connected": bool(shifted_has),
...
def annotation_audit(
    ...
    *,
    ...
    return {
        ...
        "corrected_track_shift_reference": {
        ...
        "note": (
            "The corrected-track shift shown in this initial audit is an "
            "evaluation reference only. Pass 2 uses a separate shift estimated "
            "from pass-1 Trackastra predictions."
        ),
    }


def print_audit(audit: dict[str, Any]) -> None:
    banner("INVESTIGATION 44 — TRACKASTRA BOOTSTRAP GLOBAL MOTION")
    ...
    print("")
    print("CORRECTED-TRACK SHIFT REFERENCE — EVALUATION ONLY")
    print("-" * 104)
    print(
        f"padded canvas ZYX   : "
        f"{tuple(audit['corrected_track_shift_reference']['canvas_shape_zyx'])}"
    )
    print(
        "max |alignment| ZYX : "
        f"{tuple(audit['corrected_track_shift_reference']['maximum_absolute_integer_alignment_zyx'])}"
    )
...
def print_final_comparison(comparison: dict[str, Any]) -> None:
    ...
    shifted = comparison["bootstrap_shift"]

    b_continue = baseline["primary_continue"]
    s_continue = shifted["primary_continue"]

    b_full = comparison["full_volume"]["baseline"]
    s_full = comparison["full_volume"]["bootstrap_shift"]

    banner("INVESTIGATION 44 — FINAL COMPARISON")
    ...
    print("")
    print("PRECISE ANNOTATED CONTINUE SCORE")
    print("-" * 104)
    print(
        f"{'METHOD':<26}{'RECOVERED':>20}{'STILL BROKEN':>20}"
    )
    print("-" * 66)
    baseline_recovered_text = (
        f"{b_continue['recovered_count']}/{b_continue['target_count']} "
        f"({format_pct(b_continue['recall'])})"
    )
    shifted_recovered_text = (
        f"{s_continue['recovered_count']}/{s_continue['target_count']} "
        f"({format_pct(s_continue['recall'])})"
    )

    print(
        f"{'baseline / pass 1':<26}"
        f"{baseline_recovered_text:>20}"
        f"{b_continue['missed_count']:>20}"
    )
    print(
        f"{'bootstrap shift / pass 2':<26}"
        f"{shifted_recovered_text:>20}"
        f"{s_continue['missed_count']:>20}"
    )

    print("")
    print("ROUGH 100-FRAME INTERIOR ENDPOINT DIAGNOSTIC")
    print("-" * 104)
    print(
        f"{'METHOD':<26}{'NEW':>14}{'BROKEN':>14}{'TOTAL ENDPTS':>18}{'FAIL COMPONENTS':>20}"
    )
    print("-" * 92)

    def full_line(name: str, row: dict[str, Any]) -> None:
        print(
            f"{name:<26}"
            f"{row['interior_new_endpoints']:>14}"
            f"{row['interior_broken_endpoints']:>14}"
            f"{row['total_interior_failure_endpoints']:>18}"
            f"{row['components_with_interior_failure']:>20}"
        )

    full_line("baseline / pass 1", b_full)
    full_line("bootstrap shift / pass 2", s_full)

    print("")
    print("BOOTSTRAP SHIFT ACCURACY VS CORRECTED-TRACK REFERENCE")
    print("-" * 104)
    shift = comparison["shift_reference_error"]
    print(
        f"pair error median      : {shift['pair_error_um_median']:.3f} um"
    )
    print(
        f"pair error p90         : {shift['pair_error_um_p90']:.3f} um"
    )
    print(
        f"cumulative error p90   : {shift['cumulative_error_um_p90']:.3f} um"
    )
    print(
        f"final cumulative error : {shift['cumulative_error_um_final']:.3f} um"
    )
    ...
    print(
        f"rough broken endpoints : "
        f"{b_full['interior_broken_endpoints']} -> "
        f"{s_full['interior_broken_endpoints']}"
    )
    print(
        f"rough failure endpoints: "
        f"{b_full['total_interior_failure_endpoints']} -> "
        f"{s_full['total_interior_failure_endpoints']}"
    )
    print("=" * 104, flush=True)
...
def run(args: argparse.Namespace) -> None:
    ...
    # Corrected-track reference is evaluation-only in Investigation 44.
    reference_shift = estimate_oracle_shifts(
        paths,
        truth,
        spatial_shape,
        spacing_zyx=spacing,
    ...
    atomic_csv(
        paths.output / "corrected_track_reference_shifts.csv",
        reference_shift.rows,
    )
    ...
    audit["investigation"] = 44
    audit["shift_source_for_pass_2"] = (
        "first-pass Trackastra unambiguous adjacent continuations"
    )
    audit["corrected_track_shift_is_evaluation_only"] = True
    ...
    try:
        ...
        raise RuntimeError(
            "Trackastra is required for Investigation 44. Activate the same "
    ...
    baseline_signature = digest_payload(
        {**experiment_base, "condition": "baseline_pass_1"}
    )

    print("")
    print("[trackastra] loading model once for both passes", flush=True)
    ...
    if not args.force:
        ...
        if baseline is not None:
            print("[baseline] reusing cached pass-1 Trackastra graph", flush=True)

    if baseline is None:
        banner("INVESTIGATION 44 — PASS 1 TRACKASTRA ON CURATED INSTANCES")
    ...
    # -------------------------------------------------------------------------
    # B. Estimate global shift from pass 1 only
    # -------------------------------------------------------------------------
    bootstrap_shift = estimate_bootstrap_shifts(
        baseline.graph,
        frame_count=paths.frame_count,
        spatial_shape_zyx=spatial_shape,
        spacing_zyx=spacing,
    ...
    atomic_csv(paths.shifts_csv, bootstrap_shift.rows)

    shift_error_rows, shift_error_summary = compare_shift_estimates(
        bootstrap_shift,
        reference_shift,
        spacing_zyx=spacing,
    )
    atomic_csv(
        paths.output / "bootstrap_vs_corrected_track_shift.csv",
        shift_error_rows,
    )

    shift_summary = {
        "source": "first_pass_trackastra",
        "pairwise_observed_displacement_zyx": (
            bootstrap_shift.pairwise_float_zyx.tolist()
        ),
        "cumulative_observed_displacement_zyx": (
            bootstrap_shift.cumulative_float_zyx.tolist()
        ),
        "integer_alignment_applied_zyx": (
            bootstrap_shift.align_int_zyx.tolist()
        ),
        "padded_placement_zyx": bootstrap_shift.placement_zyx.tolist(),
        "canvas_shape_zyx": list(bootstrap_shift.canvas_shape_zyx),
        "rounding": "numpy.rint(cumulative displacement), then negate",
        "wraparound": False,
        "reference_error": shift_error_summary,
    }
    atomic_json(paths.shift_summary_json, shift_summary)

    print("")
    print("BOOTSTRAP GLOBAL SHIFT")
    print("-" * 104)
    print(
        f"padded canvas ZYX   : {bootstrap_shift.canvas_shape_zyx}",
        flush=True,
    )
    print(
        f"pair error median   : "
        f"{shift_error_summary['pair_error_um_median']:.3f} um",
        flush=True,
    )
    print(
        f"pair error p90      : "
        f"{shift_error_summary['pair_error_um_p90']:.3f} um",
        flush=True,
    )
    ...
    shifted_signature = digest_payload(
        {
            **experiment_base,
            "condition": "bootstrap_shift_pass_2",
            "integer_alignment_applied_zyx": (
                bootstrap_shift.align_int_zyx.tolist()
            ),
            "canvas_shape_zyx": list(bootstrap_shift.canvas_shape_zyx),
    ...
    if not args.force:
        ...
        if shifted is not None:
            print("[bootstrap shift] reusing cached pass-2 graph", flush=True)

    if shifted is None:
        banner("INVESTIGATION 44 — PASS 2 TRACKASTRA AFTER BOOTSTRAP SHIFT")
        ...
        shifted = run_trackastra(
            ...
            metadata={
                ...
                "shift_source": "first_pass_trackastra",
                "integer_shift": True,
                "wraparound": False,
                "canvas_shape_zyx": list(bootstrap_shift.canvas_shape_zyx),
    ...
    # -------------------------------------------------------------------------
    # D. Precise annotated comparison
    # -------------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------------
    # E. Rough full-volume 100-frame endpoint comparison
    # -------------------------------------------------------------------------
    geometry = build_curated_geometry(paths)

    baseline_full, baseline_endpoint_rows = full_volume_endpoint_diagnostics(
        baseline,
        geometry,
        frame_count=paths.frame_count,
        spatial_shape_zyx=spatial_shape,
    ...
    shifted_full, shifted_endpoint_rows = full_volume_endpoint_diagnostics(
        shifted,
        geometry,
        frame_count=paths.frame_count,
        spatial_shape_zyx=spatial_shape,
    ...
    baseline_endpoint_rows.insert(0, "condition", "baseline")
    shifted_endpoint_rows.insert(0, "condition", "bootstrap_shift")
    atomic_csv(
        paths.output / "full_volume_endpoint_diagnostics.csv",
        pd.concat(
            [baseline_endpoint_rows, shifted_endpoint_rows],
            ignore_index=True,
        ),
    )
    ...
    comparison = {
        ...
        "bootstrap_shift": shifted_metrics,
        ...
        "shift_reference_error": shift_error_summary,
        "full_volume": {
            "baseline": baseline_full,
            "bootstrap_shift": shifted_full,
        },
        "bootstrap_shift_uses_annotation_ground_truth": False,
        "corrected_track_reference_used_only_for_evaluation": True,
        "rough_full_volume_metric_is_ground_truth": False,
        "interpretation_rule": (
            "Success requires improvement on explicit manual Continue edges "
            "and a consistent reduction in boundary-filtered interior endpoint "
            "failures across the full 100-frame movie."
        ),
    }
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Investigation 44: compare Trackastra on curated BioHub instances "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Investigation 44 defaults to CPU because the current cu130 "
    ...
    parser.add_argument(
        ...
        type=float,
        default=2.0,
        help="Minimum physical residual gate for bootstrap shift inliers.",
    )

    parser.add_argument(
        "--boundary-margin-um",
        type=float,
        default=float(BOUNDARY_MARGIN_UM),
        help=(
            "Near-boundary heuristic used by the annotation system. Exact "
            "six-face raster contact is additionally treated as boundary."
        ),
    )
    ...
    parser.add_argument(
        "--audit-only",
        action="store_true",
        help="Audit annotations and calculate bootstrap shifts without Trackastra.",
    )
    parser.add_argument(
        "--force",
        action="store_true",
