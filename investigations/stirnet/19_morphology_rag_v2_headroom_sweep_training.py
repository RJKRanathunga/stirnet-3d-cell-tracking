...
"""
STIR-Net Investigation 19 — morphology-v2 RAG headroom sweep training.

This stage trains morphology-v2 and compares broad contact-ROI headroom.
...
Investigation 19 therefore starts FRESH from the known-good step-1000 spatial
milestone and trains only the new morphology correction branch over diverse
Drosophila_1 + Drosophila_2 crops.

Headroom sweep
--------------
The v2 edge representation keeps a LOCAL contact ROI fixed at 50% headroom per
side. The BROAD joint A+B contact ROI is swept over:

    50%   -> final broad extent ~2x the contact-bbox extent
    100%  -> final broad extent ~3x the contact-bbox extent
    150%  -> final broad extent ~4x the contact-bbox extent

Every variant starts fresh from the same original step-1000 spatial milestone
and uses the same seed, train/validation split, optimizer, loss, and validation
policy. This isolates the effect of broad ROI context.
...
From repository root:
    ...
    python investigations/stirnet/19_morphology_rag_v2_headroom_training.py ^
        --execution local ^
        --max-steps 4 ^
        --validation-every 2 ^
        --validation-crops-per-sample 2 ^
        --run-name morphology_v2_headroom_smoke
...
Recommended first real local run:
    ...
    python investigations/stirnet/19_morphology_rag_v2_headroom_training.py ^
        --execution local ^
        --max-steps 600 ^
        --run-name drosophila_12_morphology_rag_v2_headroom
...
APP_NAME = "stirnet-morphology-rag-v2-headroom-training"
...
def _parse_headroom_list(value: str) -> tuple[float, ...]:
    rows: list[float] = []
    for token in re.split(r"[,\s]+", str(value).strip()):
        if not token:
            continue
        number = float(token)
        if not math.isfinite(number) or number < 0.50:
            raise ValueError(
                "Every broad headroom value must be finite and >= 0.50"
            )
        if number not in rows:
            rows.append(number)
    if not rows:
        raise ValueError("--headrooms must contain at least one value")
    return tuple(rows)


def _headroom_tag(value: float) -> str:
    return f"h{int(round(float(value) * 100)):03d}"
...
def _build_morphology_model(
    ...
    *,
    device: str,
    broad_headroom_fraction: float,
    local_headroom_fraction: float,
    min_headroom_dref: float,
):
    ...
    required_v2_fields = (
        "rag_edge_contact_headroom_fraction",
        "rag_edge_local_headroom_fraction",
        "rag_edge_min_headroom_dref",
    )
    missing_v2 = [
        name
        for name in required_v2_fields
        if not hasattr(model_cfg.partition, name)
    ]
    if missing_v2:
        raise RuntimeError(
            "Morphology-v2 patch is not installed. Missing PartitionConfig "
            "fields: " + ", ".join(missing_v2)
        )

    model_cfg.partition.rag_edge_contact_headroom_fraction = float(
        broad_headroom_fraction
    )
    model_cfg.partition.rag_edge_local_headroom_fraction = float(
        local_headroom_fraction
    )
    model_cfg.partition.rag_edge_min_headroom_dref = float(
        min_headroom_dref
    )
    model_cfg.validate()
    ...
    *,
    ...
    try:
        save_checkpoint(
            ...
            training_config={
                "experiment": "19_morphology_rag_v2_headroom_training",
...
def _print_header(
    *,
    ...
    print(
        "STIR-Net Investigation 19 — morphology-v2 RAG headroom training",
        flush=True,
    )
    print("=" * 122, flush=True)
    ...
    print(
        f"Broad contact headroom    : {100.0 * broad_headroom_fraction:.0f}% / side",
        flush=True,
    )
    print(
        f"Local contact headroom    : {100.0 * local_headroom_fraction:.0f}% / side",
        flush=True,
    )
    print(
        f"Minimum headroom floor    : {min_headroom_dref:.2f} * dref / side",
        flush=True,
    )
    print(
...
def _training_impl(
    *,
    ...
    broad_headroom_fraction: float = 1.0,
    local_headroom_fraction: float = 0.5,
    min_headroom_dref: float = 0.5,
    run_name: str = "drosophila_12_morphology_rag_v2_headroom_h100",
    resume: bool = False,
    seed: int = 230525,
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required for Investigation 19")
    ...
    if broad_headroom_fraction < 0.50:
        raise ValueError("broad_headroom_fraction must be >= 0.50")
    if local_headroom_fraction < 0.50:
        raise ValueError("local_headroom_fraction must be >= 0.50")
    if broad_headroom_fraction < local_headroom_fraction:
        raise ValueError(
            "broad_headroom_fraction must be >= local_headroom_fraction"
        )
    if min_headroom_dref <= 0:
        raise ValueError("min_headroom_dref must be positive")
    ...
    _print_header(
        ...
        broad_headroom_fraction=broad_headroom_fraction,
        local_headroom_fraction=local_headroom_fraction,
        min_headroom_dref=min_headroom_dref,
        amp_dtype=amp_dtype,
    ...
    config_payload = {
        ...
        "broad_headroom_fraction": float(broad_headroom_fraction),
        "local_headroom_fraction": float(local_headroom_fraction),
        "min_headroom_dref": float(min_headroom_dref),
        "legacy_rag_frozen": True,
    ...
    try:
        ...
        print("", flush=True)
        print("=" * 122, flush=True)
        print("Investigation 19 complete", flush=True)
        print("=" * 122, flush=True)
...
def _python_cli_main() -> None:
    ...
    parser.add_argument(
        ...
        default="0.5,1.0,1.5",
        help=(
            "Comma-separated BROAD contact-bbox headroom fractions. "
            "Default: 50%, 100%, 150%."
        ),
    )
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="LOCAL contact-bbox headroom kept fixed across all variants.",
    )
    parser.add_argument(
        "--min-headroom-dref",
        type=float,
        default=0.5,
        help="Minimum physical headroom per axis/per side in dref units.",
    )
    parser.add_argument(
        "--run-name",
        default="drosophila_12_morphology_rag_v2_headroom",
    )
    ...
    common_kwargs = dict(
        ...
        local_headroom_fraction=args.local_headroom,
        min_headroom_dref=args.min_headroom_dref,
        resume=args.resume,
        seed=args.seed,
    )

    headrooms = _parse_headroom_list(args.headrooms)
    if args.local_headroom < 0.50:
        raise ValueError("--local-headroom must be >= 0.50")
    if args.min_headroom_dref <= 0:
        raise ValueError("--min-headroom-dref must be positive")
    if any(value < args.local_headroom for value in headrooms):
        raise ValueError(
            "Every BROAD headroom must be >= the fixed LOCAL headroom"
        )

    print("=" * 122, flush=True)
    print("Morphology-v2 controlled headroom sweep", flush=True)
    print("=" * 122, flush=True)
    print(
        "broad variants           : "
        + ", ".join(f"{100*v:.0f}%" for v in headrooms),
        flush=True,
    )
    print(
        f"fixed local headroom     : {100*args.local_headroom:.0f}%",
        flush=True,
    )
    print(
        f"minimum headroom floor   : {args.min_headroom_dref:.2f} * dref",
        flush=True,
    )
    print(
        "control                  : same step-1000 init / seed / split / loss",
        flush=True,
    )
    print("=" * 122, flush=True)
    ...
    if args.execution == "local":
        sweep_results = []
        for ordinal, headroom in enumerate(headrooms, 1):
            tag = _headroom_tag(headroom)
            run_name = f"{args.run_name}_{tag}"
            print(
                f"\n[sweep {ordinal}/{len(headrooms)}] "
                f"BROAD={100*headroom:.0f}% "
                f"LOCAL={100*args.local_headroom:.0f}% "
                f"run={run_name}",
                flush=True,
            )
            result = _training_impl(
                **common_kwargs,
                broad_headroom_fraction=headroom,
                run_name=run_name,
                execution_mode="local",
            )
            sweep_results.append(
                {
                    "broad_headroom_fraction": float(headroom),
                    "run_name": run_name,
                    "summary": result,
                }
            )

        def variant_rank(row):
            summary = row["summary"]
            baseline = summary["baseline_validation"]
            ...
            violation = max(
                0.0,
                floor - best["positive_accept_rate"],
            )
            return (
                int(violation > 0),
                violation,
                best["false_merge_rate"],
                best["bce"],
            )

        ranked = sorted(sweep_results, key=variant_rank)
        experiment_root = (
            LOCAL_REPO_ROOT
            / "runs"
            / "stirnet"
        ...
        sweep_summary = {
            "status": "success",
            "experiment": "19_morphology_rag_v2_headroom_training",
            "headrooms": [float(v) for v in headrooms],
            "local_headroom_fraction": float(args.local_headroom),
            "min_headroom_dref": float(args.min_headroom_dref),
            "max_steps_per_variant": int(args.max_steps),
            "seed": int(args.seed),
            ...
            "ranking": [
                {
                    "rank": index + 1,
                    "broad_headroom_fraction": float(
                        row["broad_headroom_fraction"]
                    ),
                    "run_name": row["run_name"],
                    "best_step": int(row["summary"]["best_step"]),
                    "best_metrics": row["summary"]["best_metrics"],
                }
                for index, row in enumerate(ranked)
            ],
            "recommended_headroom_fraction": float(
                ranked[0]["broad_headroom_fraction"]
            ),
            "generated_utc": datetime.now(timezone.utc).isoformat(),
        }
        _atomic_json(
            experiment_root / "headroom_sweep_summary.json",
            sweep_summary,
        )

        print("\n" + "=" * 122)
        print("HEADROOM SWEEP COMPLETE")
        print("=" * 122)
        for index, row in enumerate(ranked, 1):
            metrics = row["summary"]["best_metrics"]
            print(
                f"#{index} BROAD={100*row['broad_headroom_fraction']:.0f}% "
                f"best_step={row['summary']['best_step']} "
                f"FM-rate={metrics['false_merge_rate']:.5f} "
                f"pos-accept={metrics['positive_accept_rate']:.4f} "
                f"BCE={metrics['bce']:.5f}"
            )
        print(
            "sweep summary             : "
            f"{experiment_root / 'headroom_sweep_summary.json'}"
        )
        print("=" * 122)
        return

    # Modal sweep: launch each variant as a separate durable Modal invocation.
    ...
    for ordinal, headroom in enumerate(headrooms, 1):
        ...
        command = [
            ...
            "--broad-headroom-fraction",
            str(headroom),
            "--local-headroom-fraction",
            str(args.local_headroom),
            "--min-headroom-dref",
            str(args.min_headroom_dref),
            "--run-name",
            run_name,
            "--seed",
            str(args.seed),
        ...
        print(
            f"[modal sweep {ordinal}/{len(headrooms)}] "
            + " ".join(command),
            flush=True,
        )
