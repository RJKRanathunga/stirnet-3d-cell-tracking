...
def _make_output_and_targets(shape=(2, 1, 4, 8, 8)):
    ...
    output = VectorCNNOutput(
        ...
        vectors_normalized=torch.tanh(
            torch.randn(batch, 3, z, y, x, requires_grad=True)
        ),
...
def test_multitask_loss_is_finite_and_backpropagates():
    ...
    criterion = VectorCNNLoss()

    losses = criterion(output, targets)

    assert set(losses.as_dict()) == {
        "total", "foreground", "vector", "direction", "boundary", "center"
    }
...
def test_perfect_binary_logits_have_lower_dice_loss_than_wrong_logits():
    ...
    good = soft_dice_loss_from_logits(good_logits, target)
    bad = soft_dice_loss_from_logits(bad_logits, target)
...
def test_valid_mask_removes_invalid_voxels_from_focal_loss():
    ...
    loss_a = focal_bce_with_logits(logits_a, target, valid_mask=valid)
    loss_b = focal_bce_with_logits(logits_b, target, valid_mask=valid)
