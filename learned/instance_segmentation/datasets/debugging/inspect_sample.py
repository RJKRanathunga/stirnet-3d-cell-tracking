...
def _parser() -> argparse.ArgumentParser:
    ...
    selection.add_argument(
        ...
        type=int,
        default=0,
        help="Nth buildable neighboring pair",
    )
    selection.add_argument(
        "--raw-pair-index",
        type=int,
        help="Raw neighboring-pair index before buildability filtering",
    )
    selection.add_argument(
        "--instance-ids",
        type=int,
        nargs="+",
        help="Build a sample directly from known instance IDs, bypassing adjacency search",
    )
...
def main() -> None:
    ...
    if args.instance_ids is not None:
        ...
        if len(instance_ids) == 1:
            ...
            group_kind = "pair_merge"
        else:
            group_kind = "manual_group"
    ...
    if args.napari:
        ...
        viewer.add_labels(
            sample.input_component_mask.astype(np.int32),
        ...
        if sample.marker_positions_zyx:
            viewer.add_points(
                np.asarray(sample.marker_positions_zyx),
                name="effective markers",
                size=3,
                scale=scale,
            )
