...
class DetectionMessageBlock(nn.Module):
    ...
    def forward(self, x: Tensor, edge_index: Tensor, edge_attr: Tensor) -> Tensor:
        ...
        # Also expose the reverse relationship without assuming the preprocessing
        # duplicated every candidate edge direction.
...
class TemporalGraphEncoder(nn.Module):
    """Encode the existing detection/tracklet graph into coarse temporal tokens."""
    ...
    def __init__(self, cfg: TemporalConfig):
        ...
        self.blocks = nn.ModuleList(
            [
                DetectionMessageBlock(d, cfg.edge_dim, cfg.graph_hidden_dim, cfg.dropout)
    ...
    def empty(self, device: torch.device, dtype: torch.dtype) -> TemporalState:
        ...
        return TemporalState(
            ...
            status=torch.zeros((0, self.cfg.status_dim), device=device, dtype=dtype),
    ...
    def forward(self, data: TemporalInput | None) -> TemporalState:
        ...
        if data.graph_x.shape[-1] != self.cfg.node_dim:
            raise ValueError(
                f"Temporal graph_x must have width {self.cfg.node_dim}; got {data.graph_x.shape[-1]}"
            )
        if data.graph_edge_attr.shape[-1] != self.cfg.edge_dim:
            raise ValueError(
                f"Temporal graph_edge_attr must have width {self.cfg.edge_dim}; got {data.graph_edge_attr.shape[-1]}"
            )
        ...
        if data.node_history_embedding is not None:
            ...
            gate = torch.sigmoid(self.history_gate(torch.cat([x, history], dim=-1)))
            x = x + gate * history
        ...
        counts.index_add_(0, data.tracklet_id, x.new_ones((x.shape[0], 1)))
